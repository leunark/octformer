...
parser.add_argument('--path_in', type=str, required=False, default='',
                    help='Path to pre-downloaded dataset.')
...
def download_m40():
    if args.path_in:
        print(f"-> Using pre-downloaded dataset from: {args.path_in}")
        src_folder = args.path_in
        dst_folder = os.path.join(root_folder, 'ModelNet40')
        if not os.path.exists(dst_folder):
            print(f"-> Copying dataset to: {dst_folder}")
