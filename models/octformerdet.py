...
from .octformer import OctFormer as _OctFormer, OctreeCrossAttention

class OctFormerDetectionBlock(torch.nn.Module):
   ...
   def forward(self, query_feat, memory_feat, octree, depth):
       query = self.norm(query_feat)
       return self.cross_attn(query, memory_feat, octree, depth)
...
class OctFormerSingleStage3DDetector(Base3DDetector):
   ...
   def extract_feat(self, points):
       ...
       # Add cross attention between deep and shallow features
       detect_block = OctFormerDetectionBlock(
           dim=features[-1].shape[1],
           num_heads=8,
           patch_size=32
       ).to(points[0].device)
       
       features[-1] = detect_block(features[-1], features[0], octree, depths[-1])
