...
from .octformer import OctFormer, OctreeCrossAttention
from .octformer import OctreeT

class SegHeader(torch.nn.Module):
   def __init__(self, out_channels: int, channels: List[int], fpn_channel: int,
   ...
   def forward(self, features: Dict[int, torch.Tensor], octree: Octree, query_pts: torch.Tensor):

       depth = min(features.keys())
       depth_max = max(features.keys())
       assert self.num_stages == len(features)

    #Convert to OctreeT for cross attention
       octree_t = OctreeT(octree, patch_size=32, dilation=1, nempty=True)

    #Get deepest and shallowest features
       deep_feat = self.conv1x1[0](features[depth])
       shallow_feat = self.conv1x1[-1](features[depth_max]) 
    
    #apply cross attentionn with OctreeT
       feature = self.cross_attn(deep_feat, shallow_feat, octree_t, depth)
