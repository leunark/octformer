...
class OctreeCrossAttention(torch.nn.Module):
    ...
    def forward(self, q_data: torch.Tensor, kv_data: torch.Tensor, octree: OctreeT, depth: int):
        H = self.num_heads
        K = self.patch_size
        C = self.dim
        D = self.dilation
        ...
        if pad_len > 0:
            ...
            kv_data = torch.cat([kv_data, pad], dim=0)
        ...
        # Project q and kv
        q = self.q(q_data).reshape(-1, K, H, C // H).permute(0, 2, 1, 3)
        kv = self.kv(kv_data).reshape(-1, K, 2, H, C // H).permute(2, 0, 3, 1, 4)
        k, v = kv[0], kv[1]
        q = q * self.scale
...
class OctFormerBlock(torch.nn.Module):
    def __init__(self, dim: int, num_heads: int, patch_size: int = 32,
                 ...
                 use_cross_attn: bool = False, **kwargs):
        super().__init__()
        self.norm1 = torch.nn.LayerNorm(dim)
        self.norm_cross = torch.nn.LayerNorm(dim) if use_cross_attn else None
        
        #self attention
        self.self_attn = OctreeAttention(
        ...
        self.cross_attn = OctreeCrossAttention(
            dim, patch_size, num_heads, qkv_bias,
            qk_scale, attn_drop, proj_drop, dilation) if use_cross_attn else None
