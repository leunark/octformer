...
parser.add_argument('--alias', type=str, required=False, default='scannet_crossattn')
...
def train():
  cmds = [
      'python segmentation_crossattn.py',  # Use the modified segmentation script
      '--config configs/seg_scannet_crossattn.yaml',  # Use the new config
...
def test():
  ...
  ckpt = ('logs/scannet/octformer_crossattn_{}/best_model.pth'.format(args.alias)
          if args.ckpt == '\'\'' else args.ckpt)
  cmds = [
      'python segmentation_crossattn.py',  # Use the modified segmentation script
      '--config configs/seg_scannet_crossattn.yaml',
  ...
  cmds = [
      ...
      '--path_pred logs/scannet/octformer_crossattn_test_{}'.format(args.alias),
      '--path_out logs/scannet/octformer_crossattn_test_seg_{}'.format(args.alias),
...
def validate():
  ...
  cmds = [
      'python tools/seg_scannet.py',
      '--run generate_output_seg',
      '--path_pred logs/scannet/octformer_crossattn_val_{}'.format(args.alias),
      '--path_out  logs/scannet/octformer_crossattn_val_seg_{}'.format(args.alias),
  ...
  cmds = [
      ...
      '--path_pred logs/scannet/octformer_crossattn_val_seg_{}'.format(args.alias), ]
  execute_command(cmds)
